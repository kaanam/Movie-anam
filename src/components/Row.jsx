...
const Row = ({ title, fetchURL,rowID,genre}) => {
  ...
  return (
    <div>
      <div className="flex flex-row items-center mt-8 pr-8 justify-end w-full">
      <h2 className="text-[#FF0000] font-bold md:text-xl p-8 cursor-pointer" onClick={handleClick}>{title}</h2>
      {genre ? <FiChevronsRight onClick={handleClick} className="cursor-pointer"/> : null}
      </div>
